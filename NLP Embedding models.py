# NLP Algorithms/Embeddings - Used to convert unstuctured data to structured data
import nltk
import re
...
para1=''' have three visions for India. In 3000 years of our history, people from all over
...
Our achievements are being globaly recognised today. Yet we lack the self-confidence to see ourselves as a developed nation, self-reliant and self-assured. Isn't this incorrect?
...
sent_tokens=nltk.sent_tokenize(para1)

for i in range(len(sent_tokens)):
    review=re.sub('[^a-zA-Z]',' ',sent_tokens[i])
    review=review.lower()
    review=review.split()
    review=[wnl.lemmatize(i) for i in  review if not  i in set(stopwords.words("english"))]
    review='  '.join(review)
...
# TFIDF Vectorizer Embedding model
